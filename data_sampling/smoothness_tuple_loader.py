...
class TupleLoader:
    ...
    def pkl_at_index(self, index, subset, ordered=True, batch_idx=None, lbl=None, verbose=False):
        ...
        for i in len(pkl_all):
            ...
            for j in range(const.context_channels):
                current_frame = pkl[y:y + const.frame_height, x:x + const.frame_width, rand_rgb_channel,
                                frames_order[j]];
                next_frame = pkl[y:y + const.frame_height, x:x + const.frame_width, rand_rgb_channel,
                             frames_order[j + 1]];
    ...
    def unsupervised_next(self, subset, fix_label=None):
        ...
        contexts = np.zeros((const.batch_size, 2, const.frame_height, const.frame_width, const.context_channels))
        ...
        for batch_idx in np.arange(0, const.batch_size):
            ...
            if (sampling_ratio[batch_idx] < postive_ratio):
                ...
                if (class_lbls[pos_tuple[batch_idx]] != -1):
                    ...
                    c_0, c_1 = self.pkl_at_index(pos_tuple[batch_idx], subset_name,
